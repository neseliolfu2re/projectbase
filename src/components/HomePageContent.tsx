...
type Props = {
  /** When true, show a short note that this is the Farcaster/Base mini-app entry. */
  miniEntry?: boolean
}

export function HomePageContent({ miniEntry = false }: Props) {
  return (
    <main className="min-h-screen">
      <div className="mx-auto flex max-w-2xl flex-col gap-8 px-4 pb-16 pt-10 sm:px-6 sm:pt-14">
        {miniEntry ? (
          <div className="-mb-2 rounded-xl border border-indigo-200/80 bg-indigo-50/90 px-4 py-3 text-center text-sm text-indigo-950 sm:text-left">
            <span className="font-semibold">Mini app entry:</span> same experience as the home page, with
            host splash handling. Manifests use{' '}
            <code className="rounded bg-white/80 px-1.5 py-0.5 font-mono text-xs">/mini</code> as{' '}
            <code className="rounded bg-white/80 px-1.5 py-0.5 font-mono text-xs">homeUrl</code>.
          </div>
        ) : null}
